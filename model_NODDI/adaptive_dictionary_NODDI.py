...
def display_cubes(cubes, ax):
    for cube in cubes:
        ...
        for i, axis in enumerate(axes):
            vmin, vmax = cube[f"{axis}_min"], cube[f"{axis}_max"]
            corners[i] = vmin + (vmax - vmin) * corners[i]
...
for recursion in range(recursion_level): #from 0 to 5
    ...
    for parameter in ["iso", "ic", "od"]:
        new_cubes = []
        for cube in cubes:
            param_min = cube[f"{parameter}_min"]
            param_max = cube[f"{parameter}_max"]
            param_med = 0.5 * (param_min + param_max)
            cube1 = cube.copy()
...
print(f"We compute {nb_models} signals and pairwise distances.")
signals = []

for cube in cubes:
    param = create_param_from_cube(cube)
    signals.append(signal(G_dir, param1[1], param1[2], param1[0], nb_directions))
distances = np.zeros((nb_models, nb_models))
...
with open(chemin, mode='w') as mon_fichier:
    ...
    mon_fichier_ecrire.writerow(isos)
    mon_fichier_ecrire.writerow(ics)
    mon_fichier_ecrire.writerow(ods)