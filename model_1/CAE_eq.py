...
#     recon_eq=torch.zeros(size,500)
#     for i in range(size):
#         recon_eq[i] = recon_batch[i,0]*torch.exp(-b*recon_batch[i,1])
#     return recon_eq
...
def train_loop(epoch, model, train_loader, optimizer):
    ...
    for batch, (X, y) in enumerate(train_dataloader):
        ...
        loss = loss_function(recon_eq, y)+strength*reg
        sum_loss+=loss
        loss.backward()
        optimizer.step()
...
def test_loop(epoch, dataloader, model, loss_fn, loss_threshold,indices):
    ...
    if epoch%5==0:
        print(
            f"Test Error: \n"
            f"Accuracy on S: {(100*correct):>0.1f}%"
