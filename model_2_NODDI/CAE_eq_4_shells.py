...
"""
Function for the training and the testing of a concrete autoencoder (selection layer + decoder) to predict dMRI signals using estimated parameters
Loss calculated between the ground truth signals and the etimated ones
The estimated signals are calculated using the AMICO classes from the estimated parameters from the CAE
...
#Class to check NaN or Inf in tensors
...
def signal(G_dir, vol_iso, vol_ic, od, nb_directions): #compute the total signal
    ...
    delta=37.8e-3 #s
    delta_dir=delta*torch.ones((nb_directions,1))
    smalldel=17.5e-3 #s
    smalldel_dir=smalldel*torch.ones((nb_directions,1))
...
class CAE(nn.Module):
    ...
    def forward(self, x, random, temperature, threshold):
        ...
        params=params.transpose(1,2) #N_train*N_input*N_directions
        reg=outputs["reg"]
...
threshold=0.1 #threshold for the regularization
...
def train_loop(epoch, model, train_loader, optimizer):
    ...
    for batch, (X, y) in enumerate(train_loader):
        ...
        recon_sign = []
        for i in range(size): #reconstruction of the signals
